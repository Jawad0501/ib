...
$(window).on('load', function () {
  ...
  // On load Toast
  setTimeout(function () {
    let app_name = JSON.parse("{{ json_encode(config('app.name')) }}");
    let user_name = JSON.parse("{{ json_encode(auth()->user()->name) }}");
    toastr['success'](
      `You have successfully logged in to ${app_name}. Now you can start to explore!`,
      `👋 Welcome! ${user_name}`,
      {
        closeButton: true,
        tapToDismiss: false,
